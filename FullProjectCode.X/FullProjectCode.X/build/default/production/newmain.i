...
int __adc_read(int adc_channel) {
    ADCON0bits.ADON = 1;
    ADCON0bits.CHS = (unsigned char) adc_channel;
...
void __set_pwm_freq(int f) {
    ...
    int PR2_value = (int) ((PWM_PERIOD * 20000000) / (4 * 16) - 1);
    PR2 = (char) PR2_value;
...
void __set_duty_cycle(int duty_cycle) {
    ...
    char reg_value = (char) ((dc_period * 20000000) / 16);
...
unsigned char key()
{
    ...
    if(RB4==0&&RB5!=0&&RB6!=0&&RB7!=0)
            colloc=0;
    else if(RB4!=0&&RB5==0&&RB6!=0&&RB7!=0)
            colloc=1;
    else if(RB4!=0&&RB5!=0&&RB6==0&&RB7!=0)
            colloc=2;
    else if(RB4!=0&&RB5!=0&&RB6!=0&&RB7==0)
